...
public class KafkaConsumerService {
     ...
     * Le message est automatiquement désérialisé en UserRegisteredEvent grâce à la configuration Kafka
     * 
     * Processus :
     ...
     */
    @KafkaListener(topics = "registrations.created", groupId = "notification-service", 
                   containerFactory = "registrationListenerFactory")
    public void consumeUserRegistered(UserRegisteredEvent event) {
        ...
        try {
            ...
            String registrationDateStr = event.getRegistrationDate() != null ? 
                    event.getRegistrationDate().toString() : null;
     ...
     * Le message est automatiquement désérialisé en EventCreatedEvent grâce à la configuration Kafka
     * 
     * Processus :
     ...
     */
    @KafkaListener(topics = "events.created", groupId = "notification-service",
    ...
    public void consumeEventCreated(EventCreatedEvent event) {
        ...
        log.info("   └─ eventDescription: {}...", 
                event.getEventDescription() != null ? 
                        event.getEventDescription().substring(0, Math.min(50, event.getEventDescription().length())) : "null");
        ...
        try {
            ...
            // Traiter l'événement (enregistrement en DB + notification à tous les utilisateurs)
